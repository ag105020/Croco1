...
def croco(data,Temperature):
    ...
    Do2_25=2.12e-9 #Diffusion coefficient of O2 in the water at 25C (m2/s) (should be revised with MacDougall 1967)
...
#     print("KN2fix_Ctore",Kgly)
    ...
    return nansum(PC2H4*dT/3600),nansum(Cres*dT/3600),nansum(CPo2*dT/3600),mean(O2c_v[light_period_step+(3600*4)/dT:light_period_step+(3600*8)/dT]*1000)
#############################################################################
...
for aa in Labdata:
   if i==2:
       ...
       tl=time.time()
       for j in U:
           N2fix[j],ResArray[j],PhotoArray[j],O2cArray[j] = croco(aa,Tarray[j])
...
legend(lns, labs, loc='upper left',fontsize=18)
title('All $T$ dependent')
...
ax2.set_ylim(0,160)
xlim(14,28)
Savefig3(savefolder,'AllTdepend',dpi)
...
Savetxt(O2cArray,savefolder,Condition+'O2c')

figure(14)
plot(Tarray,O2cArray)
