...
def sf00(name):
    Savefig3('Croco\\Output01\\Figs\\00Default',name,dpi)
...
def pl(fignun,name,m05,m20,d05,d20,Ylabel):
    figure(fignun)
...
# figure(1)
# plot(Th,N2fix05,color=color05)
# dp05('N2fix05all')
# plot(Th,N2fix20,color=color20)
# dp20('N2fix20all')
# xlabel(Xlabel)
# ylabel('N$_{2}$fix (fmolC$_{2}$H$_{4}$ cell$^{-1}$ h$^{-1}$)')
# xlim([0,24+xedge])
# xticks(arange(0,24+1,2))
...
pl(1,'N2fix',N2fix05,N2fix20,'N2fix05all','N2fix20all','N$_{2}$fix (fmolN cell$^{-1}$ h$^{-1}$)')
ylim(bottom=-0.08); sf00('N2fix')
pl(4,'C',C05,C20,'Ccell05all','Ccell20all','C per cell (fmol C cell$^{-1}$)')
pl(5,'N',N05,N20,'Ncell05all','Ncell20all','N per cell (fmol N cell$^{-1}$)')
pl(6,'CN',CN05,CN20,'CNcell05all','CNcell20all','Molar C:N ratio (molC molN$^{-1}$)')
...
figure(8)
# plot(Th2,FeB,color='gray')
plot(Th2,FeP,color='orange')
plot(Th2,FeN,color='brown')
plot(dTh2,dFeP,'o',color='orange')
plot(dTh2,dFeN,'o',color='brown')
xlabel(Xlabel)
ylabel('Fe allocation ($\%$)')
