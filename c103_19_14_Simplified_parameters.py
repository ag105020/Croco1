...
#    if (Da==4) or (Da==0) or (Da==2) or (Da==3):        #Plotting GroBkopf 2012 O2 5%
    if (Da==0) or (Da==3):                    #Plotting GroBkopf 2012 O2 5% and 20%
...
#     print('PI',PI)
    ...
    #OOOOOOOOOOOOOOOOOOOOOOOOOOOOO        
            ...
            #=====================
            Rnitrogenaseproduction[i]=((nighttime[i])/(14400))**3*(1/1500)*3*((-0.5)/(0.14)*(O2c_v[i]-0.046+0.14/(-0.5)))*(gly[i]/(gly[i]+Kgly*5))**3    #(186-42)(187-7)
            #=====================
            
            #(1/1500)*3*((0.5-1)/(0.186-0.046)*(O2c_v[i]-0.046)+1)
    ...
    print("CNitroge-Fe_Buffer-Fe",(1/(14400))**3*(1/500)*((-0.5)/(0.14))*(-1))
    print("P4",3)
    print("O2Cell_nitroge_cri",+0.046-0.14/(-0.5))
    ...
    if Datanumber==0:
        ...
        xlabel(Xlabel)
        ylabel('(fmolC$_{2}$H$_{4}$ cell$^{-1}$ h$^{-1}$)')
        xlim([0,24*Days])
    ...
    if Datanumber==4:
        ...
        xlabel(Xlabel)
        ylabel(r'Fe allocation ($\%$)')
        xlim([0,24*Days])
        xticks(arange(0,24*Days+1,2*Days))
        ylim(0,100)
        yticks((0,20,40,60,80,100))
        ticklabel_format(style='plain', axis='y', scilimits=(0,0))
        Savefig2(savefolder,8,dpi)
