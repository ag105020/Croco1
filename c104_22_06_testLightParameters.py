...
'''
Here using different light parameters and 
save it in Light03 folders both here and in figure folder
'''
...
def croco(data,I0):
    ...
    #OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
    if (Da==0) or (Da==1) or (Da==3) or (Da==4):                    #Plotting GroBkopf 2012 O2 5% and 20%
        return
    #OOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
    #Time setting
    #OOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
    ...
    Tau=0.3    #(s) handling time (from a704_16) (Irina: half life of RNA is less than 1 minute (30 (s)) or so)
    ...
    PI = Pmax*(1-exp(-AI*I))-1/(1+exp(-(I-I0-Width*Range)/(Width)))*BI*Pmax
    PI = Pmax*(1-exp(-AI*I)) - BI*Pmax/(1 + exp(-CI*I+DI))
    PI[PI<0] = 0
  #  print('PI',PI)
    #-----------------------------------------------------------
    ...
    #OOOOOOOOOOOOOOOOOOOOOOOOOOOOO        
            ...
            if O2c_v[i]>Ocri: 
                ...
                N2fixpotential[i]=Fenitrogenase[i]*N2fixperFe*(Ocri-O2c_v[i])/Ocri    #(187-4)(186-29)
                which2[i]=2
...
Imax = 2000
Istep = 100
Iarray = arange(0,Istep+Imax,Istep)
U = arange(size(Iarray))
N2fix = zeros(size(Iarray))
...
Xlabel='$I$ (\\textrm{\\greektext m}mol m$^{-2}$ s$^{-1}$)'

figure(11)
plot(Iarray,N2fix)
plot([25,50,100,140,190,250,375,500],array([0,0,0,4.17,3.5,3.36,4.85,2.86])*3,'o')    #Data from Masuda
xlabel(Xlabel)
ylabel('N$_{2}$fix (fmolC$_{2}$H$_{4}$ cell$^{-1}$ d$^{-1}$)')

savefolder='Croco\\Light06'
dpi=100
Savefig2(savefolder,11,dpi)
Savetxt(Iarray,savefolder,'IarrayN2fix')
Savetxt(N2fix,savefolder,'N2fix')
savetxt('Light06\\N2fix.csv',N2fix,delimiter=",",fmt='%2.8f')
savetxt('Light06\\IarrayN2fix.csv',Iarray,delimiter=",",fmt='%2.8f')

Imax2 = Imax
Istep2 = Istep
Iarray2 = arange(0,Istep2+Imax2,Istep2)
U2 = arange(size(Iarray))
#PI = zeros(size(Iarray))

Mchl=893.49             #(g / mol chlorophyll) mollar mass of chlorophyll (from a 704)
Pmax0=7                     #(g C /(g Chl h) Maximum production rate per chlorophyll (around 6 by Cullen 1990) (from a704-16 (in Healey 1985 model))
Pmax=Pmax0*Mchl/12/3600/55       #(mol C s-1 mol chl-1) carbon fixing rate (156-10) (156-15) for unit conversion) (from a704-16)
Pmax=0.006898991  #from "04 Light parameterization.xlsx"
O=0.01  #(m2 / umol photon) absorption cross section (from a704_16)
Tau=0.3    #(s) handling time (from a704_16) (Irina: half life of RNA is less than 1 minute (30 (s)) or so)
PI=Pmax*(1-exp(-O*Iarray2*Tau))     #(C mol s-1 Chl mol-1) Carbohydrate fixation rate per chlorophyll (167-1)
...
PI = Pmax*(1-exp(-AI*Iarray2))-1/(1+exp(-(Iarray2-I0-Width*Range)/(Width)))*BI*Pmax
PI = Pmax*(1-exp(-AI*Iarray2)) - BI*Pmax/(1 + exp(-CI*Iarray2+DI)) #Simplified version
PI[PI<0] = 0

figure(12)
plot(Iarray2,PI)
xlabel(Xlabel)
ylabel('$P_{Chl}$ (C mol s$^{-1}$ Chl mol$^{-1}$)')
Savefig2(savefolder,12,dpi)
Savetxt(Iarray2,savefolder,'IarrayPchl')
Savetxt(PI,savefolder,'Pchl')
savetxt('Light06\\Pchl.csv',PI,delimiter=",",fmt='%2.8f')
savetxt('Light06\\IarrayPchl.csv',Iarray2,delimiter=",",fmt='%2.8f')


figure(13)
plot(Iarray,N2fix)
ax1=gca(); ax2=ax1.twinx()
ax2.plot(Iarray2,PI,color='r')
xlabel(Xlabel)
ax1.set_ylabel('N$_{2}$fix (fmolC$_{2}$H$_{4}$ cell$^{-1}$ d$^{-1}$)')
ax2.set_ylabel('$P_{Chl}$ (C mol s$^{-1}$ Chl mol$^{-1}$)')
Savefig2(savefolder,13,dpi)
